...
class AppRoutes {
  ...
  static const String adminDashboardScreen = '/admin';
  static const String unityArScreen = '/unity-ar';

  static GoRouter get router => appRouter;
}

/// Routes reachable without a fully authenticated + approved user. The
/// sign-up-login screen itself shows the "pending approval" state
/// inline once logged in but not yet approved (see sign_up_login_screen.dart),
/// so it stays reachable rather than needing its own separate route.
const _publicRoutes = {
  AppRoutes.initial,
...
final GoRouter appRouter = GoRouter(
  ...
  redirect: (context, state) {
    ...
    if (!auth.isApproved) {
      // Logged in but pending/rejected — keep them on the auth screen,
      // which renders the pending-approval card itself once it detects
      // this state (see sign_up_login_screen.dart).
